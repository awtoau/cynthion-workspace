...
from peripherals.hyperram_controller import (HyperRAMController, T_CSM_MARGIN,
                                             T_CSM_NS)
...
def completion_bound(sync_mhz):
    ...
    **Expected worst case**: its own watchdog. The tCSM burst budget is
    `T_CSM_NS * T_CSM_MARGIN`, and tCSHI recovery plus the FSM edges either side
    of it are under 32 cycles at every clock this file runs.
    ...
    **On expiry**: the transaction is counted on `model.incomplete` and section 12
    ...
    """
    return int(1.25 * (T_CSM_NS * T_CSM_MARGIN * sync_mhz / 1000.0 + 32))
