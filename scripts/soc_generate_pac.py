...
far enough to read its memory map, which needs no toolchain and no board.
    ...
    ./scripts/soc_generate_pac.py --variant-only PATH   # just the rung's clocks

## The one part that is not committed

`SYNC_HZ` and `I2C_PRESCALE` follow `CYNTHION_SYNC_MHZ`; nothing else in the
crate does. They live in `src/variant.rs`, which `build.rs` copies into `OUT_DIR`
for `base.rs` to include -- or, with `CYNTHION_SOC_VARIANT_RS` set, the copy
`soc_run.py` wrote into that variant's build directory. A build off the default
rung then gets its own timebase without rewriting the committed crate (#480).
"""
...
def cross_check(peripherals, emit):
    ...
    for name, address in sorted(expected.items()):
        ...
        # line. One f-string with the None test inside the format spec formats
        # `None` with `:08x` and raises TypeError four frames up: the diagnostic
        # for a missing peripheral crashes instead of naming it.
        if actual is None:
...
def variant_constants():
    """`sync` in Hz and the I2C prescale, from the gateware's own arithmetic.

    THE SOLVED RATE, not `SYNC_MHZ`: the first is what the dividers produce, the
    second what the design asks the PLL for. `prescale_for` is the gateware's own
    function, called with the gateware's own clock.
    """
    import top as soc_module
    ...
    return sync_hz, prescale_for(sync_hz, soc_module.I2C_SCL_HZ), soc_module.I2C_SCL_HZ


def render_variant():
    """The constants that follow `CYNTHION_SYNC_MHZ`, as one includable file.

    Separate from `base.rs` because the rest of the crate is variant-independent
    and committed, and these are not: a build off the default rung needs its own
    (#480). `build.rs` chooses which copy `base.rs` includes.
    """
    sync_hz, prescale, scl_hz = variant_constants()
    return "\n".join([
        ...
        "",
        f"/// I2C prescale for {scl_hz // 1000} kHz SCL at that clock, from the",
        ...
        f"pub const I2C_PRESCALE: u16 = {prescale};",
        "",
    ])


def render_bases(peripherals):
    ...
    # `SYNC_HZ` and `I2C_PRESCALE` follow the build variant, so they are not in
    # this file. `build.rs` picks the copy: the committed default, or the one
    # `soc_run.py` wrote into this variant's build directory (#480).
    lines += [
...
def write_bases(peripherals, emit):
    """`base.rs` plus the default variant's constants beside it."""
    for path, text in ((OUT / "src" / "base.rs", render_bases(peripherals)),
                       (OUT / "src" / "variant.rs", render_variant())):
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(text)
        emit(f"wrote {path.relative_to(ROOT)}")
    (OUT / "build.rs").write_text(BUILD_RS)


def check_bases(peripherals, emit):
    """Diff the whole generated file, not a list of names.

    A list only checks what someone remembered to put in it -- `SYNC_HZ` and
    `I2C_PRESCALE` were stale on a green gate for exactly that reason (#472).
    """
    ok = True
    for path, text in ((OUT / "src" / "base.rs", render_bases(peripherals)),
                       (OUT / "src" / "variant.rs", render_variant()),
                       (OUT / "build.rs", BUILD_RS)):
        if not path.exists() or path.read_text() != text:
            emit(f"  {path.relative_to(ROOT)} is STALE -- run without --check")
            ok = False
    emit(f"committed base.rs, variant.rs and build.rs: "
...
def write_hardware(peripherals, platform, emit):
    ...
    Size is not an argument against it: these are `.rodata`, which since #162 stage
    two lives in a 3392 KiB flash region rather than competing for 63 KiB of block
    RAM.
    """
    lines = [
...
BUILD_RS = '''// Choose which variant constants `base.rs` includes. Generated by
...
def main():
    ...
    parser.add_argument("--variant-only", metavar="PATH",
                        help="write only the variant-dependent constants to "
                             "PATH and stop; touches nothing committed")
    args = parser.parse_args()

    OUT.mkdir(parents=True, exist_ok=True)

    # No SoC elaboration and no svd2rust: these come from `top.py`'s own module
    # constants and the gateware's clock solver, which is all a rung changes.
    if args.variant_only:
        path = Path(args.variant_only)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(render_variant())
        sync_hz, prescale, _ = variant_constants()
        emit(f"wrote {path} -- SYNC_HZ {sync_hz} I2C_PRESCALE {prescale}")
        emit()
        return 0
...
def run(args, emit):
    ...
    if args.check:
        ...
        bases_current = check_bases(peripherals, emit)
        return 0 if current and offsets_current and bases_current else 1
