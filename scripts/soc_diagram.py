...
drift: add a peripheral to `top.py`, rerun this, and it is in the picture,
...
SOC_SOURCE = ROOT / "gateware" / "soc" / "top.py"
...
def elaborate(emit):
    ...
    # whose ports are declared in `cpu/cpu.py`, not read back from the Verilog, so its
    ...
    soc = soc_top.AwtoSoc(firmware=[0] * 16)
    ...
    return design, platform, requests, soc_top
