...
def line_from(reply, keys):
    ...
    while i < len(text):
        ...
        i += 1
    flat = "".join(out)
    # The echoed line is the one immediately before the dispatcher's reply.
    # Anchoring on "zz" was wrong: a case that edits the front of the line can
    # leave residue from the previous prompt in front of it, and a parser that
    # then returns "" reads as a firmware failure. Take the LAST prompt that has
    # text after it and is followed by a reply.
    lines = flat.splitlines()
    for i, piece in enumerate(lines):
        if "unknown command" in piece and i > 0:
            prev = lines[i - 1]
            at = prev.rfind("aux> ")
            if at >= 0:
                return prev[at + 5:].rstrip("\r\n").lstrip("?")
    return ""
...
def main():
    ...
    # 0.6 s. The commands here are all unknown and reply in microseconds, but
    # the reply arrives AFTER a burst of ANSI the board emits while editing --
    # up to four sequences per keystroke for a kill -- and at 0.25 s the read
    # returned mid-burst and the parser saw no reply to anchor on. That read as
    # a firmware failure and was not one.
    budget_s = 0.6
