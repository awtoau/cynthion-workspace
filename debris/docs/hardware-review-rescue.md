# Hardware work rescued from standalone fork clones

Retired to `debris/` from `docs/hardware-review/`. It is a quarantine of
uncommitted work from clones that were deleted, which is what `debris/` is for --
`docs/` is for documentation.

**Two things were dropped rather than moved.** `cynthion-hardware-exports/` was
4.7 MB of `cynthion.step` and two PNG renders, which this file itself describes
as regenerable from the PCB; regenerable content gets deleted, not archived.
And `moondancer-logport.diff` was byte-identical to
`docs/apollo/pending-patches/moondancer-uart0-log-port.diff`, whose README
already recommends deleting it -- it is a one-line debug flip that REMOVES a log.
