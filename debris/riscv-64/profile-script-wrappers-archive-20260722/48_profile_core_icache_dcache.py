#!/usr/bin/env python3
"""Profile a core-only VexiiRiscv config with 4 KiB I-cache + 4 KiB D-cache.

Flow:
1) Generate VexiiRiscv.v with fetch L1 and LSU L1 enabled
...
LOG = OUT / "core_icache_dcache_profile.log"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--tag", default="core_x64_sv_rvm_rvc_rdtime_i4k_d4k", help="Datapoint tag for metrics")
    parser.add_argument(
        "--notes",
        default="Core x64 supervisor+rvm+rvc+rdtime with 4KiB I-cache and 4KiB D-cache",
...
def generate_core_icache_dcache() -> None:
    sbt_arg = (
...
def main() -> int:
    ...
    print("Core I-cache + D-cache profile complete")
    print(f"Log: {LOG}")
