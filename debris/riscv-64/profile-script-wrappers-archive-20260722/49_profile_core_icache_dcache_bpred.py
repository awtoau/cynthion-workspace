#!/usr/bin/env python3
"""Profile core-only RV64 with 4 KiB I-cache + 4 KiB D-cache + branch prediction.

Cumulative on top of add-dcache-4k path by enabling BTB/GShare/RAS.
Flow:
1) Generate VexiiRiscv.v with fetch L1 + LSU L1 + branch prediction
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tag",
        default="core_x64_sv_rvm_rvc_rdtime_i4k_d4k_btb_gshare_ras",
        help="Datapoint tag for metrics",
    )
    parser.add_argument(
        "--notes",
        default="Core x64 supervisor+rvm+rvc+rdtime with 4KiB I-cache, 4KiB D-cache, BTB, GShare, and RAS",
...
def generate_core_icache_dcache_bpred() -> None:
    sbt_arg = (
...
def main() -> int:
    ...
    try:
        generate_core_icache_dcache_bpred()
