...
module tb;
  ...
  // A linear burst write of the ramp, at the latency the HOST believes.
  task write_burst_ramp(input [31:0] word_addr, input integer n, input integer lat);
    integer k;
  ...
  task read_burst_ramp(input [31:0] word_addr, input integer n);
    integer k;
    reg [15:0] w;
    begin
      burst_bad = 0;
  ...
  initial begin
    ...
    write_burst_ramp(BURST_BASE, BURST_WORDS, 14);
    set_latency(4'd2, 1'b0);                          // variable, L = 7
    burst_long  = 0;
    burst_wrong = 0;
    for (i = 0; i < BURST_HUNT_N; i = i + 1) begin
      read_burst_ramp(BURST_BASE, BURST_WORDS);
      if (lat_ck > 1.5 * 7) begin
