...
between two domains that worked while both were 60 MHz and then produced correct
counters with dropped characters once they differed; that is what a continuous
crossing costs, and there is no continuous crossing here.
"""
...
class BistCsrTransport(wiring.Component):
    ...
    #: sit below it. The firmware's `bist.rs` carries the same constant, and
