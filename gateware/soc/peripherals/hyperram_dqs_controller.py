...
class HyperRAMDQSController(Elaboratable):
    ...
    def __init__(self, *, phy, sync_mhz, high_latency_clocks=None,
        ...
        # cycles, so the burst ends one cycle before the watchdog would. Reaching
        # it means the device kept up and the burst was too long for tCSM; the
        # watchdog behind it firing means the device stopped. A beat is 32 bits
        # here, so the word cap is twice this in device words.
        #
        # `bootram.hyperram_max_burst_words` caps a Wishbone burst from the same
        # tCSM figure, so a caller that respects its own cap never reaches this.
    ...
    def elaborate(self, platform):
        ...
        with m.FSM() as fsm:
            ...
            with m.State('READ_DATA'):
                ...
                # above needs `phy.datavalid`, so until this the state had no exit
                # a silent device could reach -- and nothing bounded the burst a
                # live one could hold CS# Low for. Stops CK the way the ordinary
                # exit does.
