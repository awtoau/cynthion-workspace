...
## There is no input selection here, and there never was -- #305

`control_vbus_in_en` (K13) and `aux_vbus_in_en` (L13) exist on the board and are
**not driven by this design**: `top.py` does not request either pad, so neither
appears among the 126 `LOCATE` lines of the built `top.lpf`. An `input` register,
two ports, `vbus input`, `prefer_control` and `allow_all_inputs` all existed and
all terminated in an unconnected pin -- writing them and observing nothing was
indistinguishable from writing them and them working. All removed.

Overvoltage shutoff of an input is automatic in hardware above 5.5 V (`D17`, a
5.6 V zener), which is why nothing here needs to command a power input closed.
