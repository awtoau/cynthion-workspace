...
| `hyperram/hyperram_burst_test.py` | scripted single-word and four-word burst sequence | syntax; helper is broken | simulation **fails data verification but exits zero** | `hyperram_burst_test_sim.py`, `test_hyperram_build.py` | retire; helper contains placeholder constraints and an unimplemented JTAG result step |
...
| `loader/bitstream_sink.py` | USB bulk sink and transfer counters for fast-loader work | syntax | recorded USB throughput result | `scripts/fast_loader.py` | retained transport fixture; product ID comes from `usb_ids.py` |
