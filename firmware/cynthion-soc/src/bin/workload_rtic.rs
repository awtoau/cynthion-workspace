...
//!
//! `docs/rtic.md` §6 names this as the cheapest next
...
///
/// Empty because there is no power stack in this binary to refresh, and pending
/// a SLIC source that no task is bound to would be a store to nothing. Same
/// stand-in as `mod metrics` above, for the same reason.
mod rtic_app {
    pub fn tick() {}
}
...
///
/// `docs/rtic.md` §2 measured the hand-written
...
///
/// The question `docs/rtic.md` §3 leaves open is not how
...
/// `src/bin/usb_rtic.rs`'s; `docs/rtic.md` §3 is why it is written out
