...
///
/// Pure arithmetic and `core::fmt`: no inline asm, no MMIO, and `std` is
/// available because `no_std`/`no_main` are `cfg_attr(not(test))` in
/// `src/main.rs`.
///
/// **These do not run yet.** `cargo test --target x86_64-unknown-linux-gnu`
/// still fails compiling `src/selftest.rs` -- `csrr`, `.option rvc`, `c.li` are
/// not x86 instructions, and `binary_asm_labels` denies the `1:` label before
/// that. #337 is what makes the harness reach this module; these assertions
/// pass today when the block is built standalone.
#[cfg(test)]
mod tests {
    use super::Hz;

    fn show(hz: u32) -> String {
        format!("{}", Hz(hz))
    }
