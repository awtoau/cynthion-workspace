...
///
/// Pure arithmetic and `core::fmt` -- no inline asm and no MMIO, so this builds
/// for the host where the rest of the crate's tests cannot (#337). `no_std` and
/// `no_main` are both `cfg_attr(not(test))` in `src/main.rs`, so `std` and
