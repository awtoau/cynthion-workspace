...
//!     writes out of `CynthionSoC.decoder.bus.memory_map` -- the SoC's own
...
pub const UART_BASES: &[usize] = &[
    ...
    // comment on APOLLO_UART_BASE in gateware/soc/top.py for why
...
/// sources start at 1. These come from `CynthionSoC.interrupt_sources`, which is
...
/// wrapper (`rdtime` in `gateware/soc/cpu/cpu.py`), so it is the `sync`
