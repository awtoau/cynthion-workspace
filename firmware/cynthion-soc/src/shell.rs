...
pub(crate) const HELP: &[(&str, &str)] = &[
    ...
    ("bist mode [stage|bist]", "who drives the part; staging or the engine"),
    ...
    ("bram status", "where and how big; fabric RAM has no identity"),
    ...
    ("flash status", "where, how big, what it is, and its status register"),
    ...
    ("flash sfdp", "the SFDP header: signature, revision, headers"),
    ("flash status", "status register 1: BUSY and the write-enable latch"),
    ("flash erase <hex>", "erase the reserved 4 KiB sector, and nothing else"),
    ("flash program <hex>", "a seeded ramp into that sector, verified back"),
    ...
    ("hyperram status", "identity, configuration, geometry and the DQS read path"),
