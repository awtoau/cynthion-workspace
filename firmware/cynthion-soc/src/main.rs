...
impl Shell {
    ...
    fn interject(&mut self, index: usize, f: impl FnOnce(&mut Uart)) {
...
fn housekeeping(shells: &mut [Shell; MAX_CONSOLES], console: &mut Uart, devices: &mut Devices) {
    ...
    if events::pending() {
        shells[0].interject(0, events::drain);
    }
    ...
    if uart::errors_pending() {
        shells[0].interject(0, uart::report_errors);
    }
    ...
    if let Some(bus) = devices.bus.as_mut() {
        if irq::type_c_pending() {
            ...
            shells[0].interject(0, |uart| type_c.service(uart, bus));
        }
