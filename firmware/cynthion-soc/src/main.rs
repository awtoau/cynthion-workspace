...
fn boot() -> Devices {
    ...
    //
    // It ends inside `#[init]` rather than as a task, and that is the whole
    // ordering guarantee: `devices` does not exist as a shared resource until
    // `#[init]` returns, so nothing can run against a half-established board.
...
fn panic(info: &PanicInfo) -> ! {
    ...
    // panicked mid-`vbus control` would go on passing host power to an attached
