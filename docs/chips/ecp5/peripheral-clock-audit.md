...
| [`peripherals/i2c_master.py`](../../../gateware/soc/peripherals/i2c_master.py) | `sync` only | `FFSynchronizer` on SDA | `PRER`, generated from `prescale_for` | **DEFECT 4 — CLOSED** |
...
## Defect 4 — the I²C bit period is a firmware constant nothing checks — **CLOSED**

**Fixed.** `I2C_PRESCALE` is now generated by
[`../../../scripts/soc_generate_pac.py`](../../../scripts/soc_generate_pac.py)
from the gateware's own `prescale_for(sync_hz, I2C_SCL_HZ)`, exactly as this
section proposed, so the two dead references in `top.py` are live and a `sync`
change rescales nothing silently.

**And the rate was wrong for a second reason this section did not catch.** The
argument below — that 100 kHz puts `t_SU;STA` inside a standard-mode minimum —
is correct and answers the wrong question. **Every device on that bus is a
Fast-mode Plus part**: two FUSB302Bs at 1 MHz and a PAC1954 at 1 MHz, which is in
fact a 3.4 MHz part. The bus is at **1 MHz** now
([`d820d9e`](https://github.com/awtoau/cynthion-workspace/commit/d820d9e)) and
the CPU spent on the power poll fell from 6.34% to 2.70%. The audit trail is
[`../bus-speed-audit.md`](../bus-speed-audit.md); the per-part figures are in
[`../fusb302b-type-c.md`](../fusb302b-type-c.md) and
[`../pac1954-power-monitor.md`](../pac1954-power-monitor.md).

The original finding, kept because the mechanism it describes is the one that was
fixed:
...
## Defect 5 — the HyperRAM path takes `SYNC_MHZ`, not `actual_sync_mhz` — **CLOSED**

**Fixed in [`9d018e5`](https://github.com/awtoau/cynthion-workspace/commit/9d018e5),
"soc: every period from a real clock".** `top.py:1106-1107` now reads
`car.actual_sync_mhz` on both branches, with the reason spelled out two lines
above it. The two adjacent frequency assumptions below — `CR0` latency tracking
CK, and `HyperRAMPHY`'s fixed 2 ns `p_DEL_VALUE` — are unchanged and still stand.
...
## Defect 6 — two `sync`-derived periods in `top.py` read the request — **CLOSED**

**Fixed in the same commit.** Both now read the solved rate:
`top.py:1333` computes the Apollo UART divisor from `car.actual_sync_mhz`
(520 at 60 MHz, a 0.03% baud error against the ~2% a UART tolerates), and
`top.py:1446` does the same for the heartbeat.
...
| defects that are live today | 3 (1, 2, 3) |
| defects that are latent until a flag flips | 3 (7, 8, 9) |
| **closed** | **3 (4, 5, 6)** — and defect 4's rate was wrong twice over |
