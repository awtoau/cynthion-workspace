...
recorded what the chip was** before [#109](https://github.com/awtoau/cynthion-workspace/issues/109) — throughput had been characterised in
...
**The die is characterised past 200 MHz; the package is not.** Two vendor
documents outside the datasheet say so — the 2025 app note §6.4.9 lists
`CR0[7:4] = 0010b` (7 clocks) as legal *"@ 250MHz Max Frequency"* for this
generation, and Winbond's own Verilog model carries a 250 MHz AC block
(`tACC` 28 ns = 7 × 4 ns) whose default is 250 MHz for KGD against 200 MHz for
the package. Neither is permission to run the fitted `6I` there; both say the
failure at 200 MHz is not the die running out of characterisation.
...
**But CK 166 is outside the FPGA's I/O buffer, and that was missed.** ECP5
datasheet FPGA-DS-02012-1.9 **Table 3.21, Maximum I/O Buffer Speed**, gives
**LVCMOS33 a maximum output frequency of 150 MHz** (200 MHz input; note 6,
*"Maximum data rate equals 2 times the clock rate when utilizing DDR"*, makes
that 300 Mb/s out and 400 Mb/s in per pin). Every signal in this resource is
`IO_TYPE="LVCMOS33"`, and CK itself is `LVCMOS33D` — *emulated* differential,
which Table 3.21 note 2 declines to give a frequency at all, leaving the
single-ended 150 MHz as the only figure to reason from.

**So the ceiling on this board is CK 150, not the part's 166**, and the binding
constraint is an FPGA pin rather than the RAM:

| | CK | pin rate | 128-word burst at 87.7% |
|---|---|---|---|
| the `6I` part's own bin | 166 MHz | 332 MB/s | 291 MB/s |
| **the LVCMOS33 output maximum** | **150 MHz** | **300 MB/s** | **263 MB/s** |
| the `5I` substitution, if ever fitted | 200 MHz | 400 MB/s | — *further past the pin, not closer to it* |

The last row is the part that matters for a future board: **fitting a faster
HyperRAM buys nothing while the FPGA side is LVCMOS33.** The lever is the I/O
standard, and it is set by the RAM being a 3.3 V part — which is the same
argument as fitting U15 for a 1.8 V bus, now with a second reason.
[`../ecp5/lfe5u-12f.md`](../ecp5/lfe5u-12f.md) §2 has the full table and the
slew-rate note; [`../bus-speed-audit.md`](../bus-speed-audit.md) puts it beside
...
after every acknowledgement ([#185](https://github.com/awtoau/cynthion-workspace/issues/185), [`../soc-memory-bus.md`](../../soc-memory-bus.md)).
...
nothing. **[#230](https://github.com/awtoau/cynthion-workspace/issues/230)** is the work; `bist-plan.md` is the method.
...
|---|---|---|---|
| 1 | **coalesce the Wishbone burst** ([#185](https://github.com/awtoau/cynthion-workspace/issues/185)) | 12.0 → 76.8 MB/s on a cache miss, **6.4×** | the largest item by an order of magnitude. Blocked on the master bubbling a cycle after each ack, which a HyperBus data phase cannot absorb |
| 2 | **DQS at CK 120** | 105 → 210 MB/s streaming, **2×**, at the fabric clock already built | needs `fast`, the 32-bit beat assembly bypassed in `bootram.py`, and `READCLKSEL` calibrated ([#148](https://github.com/awtoau/cynthion-workspace/issues/148)) |
| 3 | **DQS at CK 150**, fabric 75 MHz | 210 → 263 MB/s, +25% | **the ECP5's LVCMOS33 output maximum, and it arrives before the part's 166.** A `sync` decision, not a HyperRAM one |
| 3b | ~~DQS at CK 166~~ | ~~+39%~~ | **out of spec on the FPGA pin** — Table 3.21 stops at 150 MHz output. Reachable only by running the buffer past its published figure |
...
| 8 | fit the `5I` bin | 332 → 400 MB/s theoretical, **0 in practice** | rework, and it moves the part *further* past the 300 MB/s the FPGA pin publishes. Worth something only as a diagnostic |
...
| streaming read, DQS, 128-word bursts | 291 MB/s @ CK 166 | **263 MB/s** @ CK 150 (fabric 75) — **the ECP5's LVCMOS33 output maximum, not the part's bin** | **never measured** | — | `HYPERRAM_DQS` is `False`; [#148](https://github.com/awtoau/cynthion-workspace/issues/148), [#186](https://github.com/awtoau/cynthion-workspace/issues/186) |
...
| CPU cache-line refill, 64 B | 213 MB/s @ CK 166 | **12.0 MB/s** @ CK 60, uncoalesced | **never measured** | — | **[#185](https://github.com/awtoau/cynthion-workspace/issues/185) — 6.4×, and the largest item here** |
...
**CR0 `0x8f2f` is not "latency 2".** `0010b` is the *code*; table 10 reads it as
**7 clocks**, which is why fixed latency costs 14. There is no 2-clock setting on
this part — the range is 3 to 7.
...
read. Queued for the Rust CLI ([#109](https://github.com/awtoau/cynthion-workspace/issues/109)) — a CPU on the bus reads all eighteen in
microseconds.
...
different mechanisms, which a non-invertible pattern scores alike as "errors".
...
The "only `01b` is defined" reading of CR1[1:0] above is confirmed across the
family by Winbond's 2025 app note §6.5.7 — the 1 µs alternative exists on the
256 Mb and later parts, never on this one.
...
8 MiB cached and executable CPU window at `0x20000000` ([#90](https://github.com/awtoau/cynthion-workspace/issues/90)). `main=1, exe=1` in the
...
trace; recovering the burst is [#185](https://github.com/awtoau/cynthion-workspace/issues/185).
...
The DQS path ([#92](https://github.com/awtoau/cynthion-workspace/issues/92)) is upstream's controller with our PHY under it, which is the
...
**tCSHI, CS# high between transactions.** `T_CSHI_NS = 10.0` is what both
controllers carry, and it is the **100 MHz** figure — the part needs **6 ns** at
166 MHz and above ([`config-ac.md`](config-ac.md)), which is one sync cycle up to
166 MHz and two above it. Either figure is a gap the design has to keep, and
`HyperRAMDQSInterface`'s `RECOVERY` state carries
...
`extra_latency | 1` — reported as a defect in [#90](https://github.com/awtoau/cynthion-workspace/issues/90) — is therefore the correct
...
| `scripts/soc_hyperram_sim.py` | the controller and the SoC above it; asserts no fact about the part ([#346](https://github.com/awtoau/cynthion-workspace/issues/346)) |
...
See [#186](https://github.com/awtoau/cynthion-workspace/issues/186) and [#188](https://github.com/awtoau/cynthion-workspace/issues/188). `scripts/hyperram_ceiling.py` is the instrument.
...
control — a modest gain, and one that survives a negative control.**
...
3. **RWDS must actually be sampled during the CA period — it is not today.**
   ...
   defect [#90](https://github.com/awtoau/cynthion-workspace/issues/90) called it. That stands — but it is correct only because of a
...
**Winbond's own model names all four values**, and makes `10b` the POR above 85 °C
alongside tCSM 1 µs — so `01b` is what a cool part *reports*, not the only value
that exists. Still read-only, so this buys no longer tCSM; it may buy a way to
*detect* the short one. [`config-ac.md`](config-ac.md).
...
moment [#90](https://github.com/awtoau/cynthion-workspace/issues/90) puts a CPU on this bus. Hybrid burst is *"one wrapped burst followed
...
restarts on the critical word. Nothing to do until [#90](https://github.com/awtoau/cynthion-workspace/issues/90) lands; worth deciding
...
### Option 5 — differential clock (`CR1[6] = 0`). Untried, and the board is already wired for it

**This is not on the brief's list and it should be.**

Winbond's *HyperRAM Application Note* (rev P01, 2025-05-28, §6.5.5;
`sources/Winbond-AN-HyperRAM-20250528.pdf`) settles the two questions that made
this look speculative:

- **Our generation supports it.** The family table lists HyperRAM 2.0
  `W956x8Mxxxxx` — this part — as *"Single-Ended (CK) or Differential (CK/CK#)"*.
  Only the 2.1 and 2.2 generations are single-ended-only.
- **The probe is safe and self-reporting.** *"For the HyperRAM device do not
  support differential clock inputs, write CR1 Bit[6] to 0 will have no effect on
  the CR1 Bit[6]"* — so write it, read it back, and the readback answers whether
  the mode took.
...
**Section 2 does not list both.** Order Information in rev A01-006
