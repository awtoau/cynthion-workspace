...
| streaming read, DQS, 128-word bursts | 291 MB/s @ CK 166 | **263 MB/s** @ CK 150 (fabric 75) — **the ECP5's LVCMOS33 output maximum, not the part's bin** | **never measured** | — | `HYPERRAM_DQS` is `False`; #148, #186 |
...
| `scripts/soc_hyperram_sim.py` | the controller and the SoC above it; asserts no fact about the part (#346) |
