...
CR0[7:4] sparse latency encoding and CR0[3] doubling, RWDS as the extra-latency
request (tri-stated until tDSV) and as read strobe and write mask, linear /
wrapped / hybrid bursts, a refresh-forced 2x election, deep power down with
RESET# recovery, and a tCSM check.

**Does not model:** setup/hold, tRWR, tRP/tRPH, hybrid sleep, software reset, or
anything analogue. Those stay the vendor model's job, which is why the pair is
the deliverable rather than either one alone.
...
| variable latency, all five codes, read and write | yes | yes | latency measured in CK, not assumed |
