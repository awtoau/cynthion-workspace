...
static sercom_t *sercom = SERCOM2;
...
void uart_initialize(bool configure_pinmux, unsigned long baudrate)
{
	...
	// Enable our receive interrupt.  On this board the DMA path takes over RX
	// (uart_dma_init clears RXC in INTENCLR), but we keep SERCOM2_IRQn active
	// for error-flag handling (BUFOVF/FERR/PERR) in SERCOM2_Handler.
	...
	// Initialise DMA ping-pong RX.  This disables the SERCOM RXC interrupt
	// (DMA handles bytes now) and enables the DMAC channel + IRQ.
	uart_dma_init();
 ...
 *
 * RXC handling is kept here for completeness but should not fire during normal
 * operation once uart_dma_init() has disabled INTENSET.RXC; DMA owns the
 * receive path.  The error-flag check below (BUFOVF/FERR/PERR) remains active
 * since those flags are independent of the DMA path.
 */
void SERCOM2_Handler(void)
{
	// If we've just received a character, handle it.
	// Under normal operation RXC is masked by uart_dma_init(); this branch
	// is a fallback for the window before DMA is initialised.
	if (sercom->USART.INTFLAG.bit.RXC)
	{
		// Read the relevant character, which marks the RXC interrupt
		// as serviced (reading DATA clears RXC).
		uint16_t byte = sercom->USART.DATA.reg;
		uart_byte_received_cb(byte);
	}

	// Check and clear any UART error flags.
	// BUFOVF (buffer overflow), FERR (framing error), PERR (parity error)
	// are write-1-to-clear.  If BUFOVF is left set, the SERCOM will keep
	// re-asserting the interrupt immediately after we return, effectively
	// locking up the CPU in the handler.
	uint16_t status = sercom->USART.STATUS.reg;
	if (status & (SERCOM_USART_STATUS_BUFOVF | SERCOM_USART_STATUS_FERR | SERCOM_USART_STATUS_PERR)) {
		sercom->USART.STATUS.reg = status; // write-1-to-clear
	}
}
